...
router.post('/startCrawling', withAuth, async function (req, res, next) {
  try {
    ...
    var avatarData = await getRandomFictitiousUser.getRandomAvatar("humhub");
...
router.get('/filterPosts', withAuth, async function (req, res, next) {
  ...
  profile.findOne({ userName: req.query.userName, socialMedia: req.query.socialMedia }).populate({
    path: 'posts',
    match: { postContent: { "$regex": req.query.filter, "$options": "i" } }
