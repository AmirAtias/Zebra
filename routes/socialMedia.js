...
router.post('/startCrawling', withAuth, async function (req, res, next) {
  try {
    ...
    global.logger.error({ meta: { error: error.message } })
  }
});
...
router.get('/filterPosts', withAuth, async function (req, res, next) {
  ...
  profile.findOne({ userName: userJson.userName, crawlingTime: userJson.crawlingTime, socialMedia: req.query.socialMedia }).populate({
    path: 'posts',
    match: { "$or": [{ postContent: { "$regex": req.query.filter } }, { comments: { "$elemMatch": { commentContent: { "$regex": req.query.filter } } } }] }
